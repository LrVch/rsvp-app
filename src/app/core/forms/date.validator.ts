...
export function dateValidator(): ValidatorFn {
    return (control: AbstractControl): { [key: string]: any } => {
        ...
        const monthLengthArr: Array<number> = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
        // Object to return if date is invalid
        const invalidObj = { 'date': true };
        // Parse the date input to integers
        const dateArr: Array<string> = dateStr.split('/');
        const month: number = parseInt(dateArr[0], 10);
        const day: number = parseInt(dateArr[1], 10);
        const year: number = parseInt(dateArr[2], 10);
        // Today's date
        const now = new Date();
